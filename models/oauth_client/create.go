...
func Create(form *apiValidator.CreateForm) (*models.OauthClient, error) {
	client := &models.OauthClient{
		Name:              form.Name,
		RedirectURIs:      strings.Join(form.RedirectURI, "$!$"),
		GrantTypes:        strings.Join(form.GrantTypes, "$!$"),
		ResponseTypes:     strings.Join(form.ResponseTypes, "$!$"),
		Scope:             form.Scope,
		Owner:             form.Owner,
		...
		Contacts:          strings.Join(form.Contacts, "$!$"),
	}
