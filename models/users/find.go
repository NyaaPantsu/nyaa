...
func FindByID(id uint) (*models.User, int, error) {
	var user = &models.User{}
	if models.ORM.Preload("Notifications", func(db *gorm.DB) *gorm.DB { return db.Order("notifications.id DESC") }).Last(user, id).RecordNotFound() {
...
func SessionByID(id uint) (*models.User, int, error) {
	var user = &models.User{}
	if models.ORM.Preload("Notifications", func(db *gorm.DB) *gorm.DB { return db.Order("notifications.id DESC") }).Where("user_id = ?", id).First(user).RecordNotFound() { // We only load unread notifications
...
func FindForAdmin(id uint) (*models.User, int, error) {
	var user = &models.User{}
	if models.ORM.Preload("Notifications", func(db *gorm.DB) *gorm.DB { return db.Order("notifications.id DESC") }).Preload("Torrents").Last(user, id).RecordNotFound() {
		return user, http.StatusNotFound, errors.New("user_not_found")
	}
	var liked, likings []models.User
	models.ORM.Joins("JOIN user_follows on user_follows.user_id=?", user.ID).Where("users.user_id = user_follows.following").Group("users.user_id").Find(&likings)
